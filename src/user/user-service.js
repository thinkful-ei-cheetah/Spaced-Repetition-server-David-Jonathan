...
const UserService = {
  ...
  validatePassword(password) {
    ...
    if (!REGEX_UPPER_LOWER_NUMBER_SPECIAL.test(password)) {
      return 'Password must be at least 8 characters long, contain one upper case, lower case, number and special character'
    }
    return null
  },
  ...
  populateUserWords(db, user_id) {
    return db.transaction(async trx => {
      ...
      const languageWords = [
        ...
        ['hej', 'hello', 3],
        ['hus', 'house', 4],
        ['utvecklare', 'developer', 5],
        ['bil', 'car', 6],
        ['fantastiskt', 'amazing', 7],
        ['vatten', 'water', 8],
        ['katt', 'cat', null],
      ]
